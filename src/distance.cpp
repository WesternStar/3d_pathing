...
//seed = grid index of destination location (where distance = 0)
{
  ...
  queue.rewind();
  while (queue.getNext(p)) {
    k=p/Z; j=(p-k*Z)/Y; i=p-k*Z-j*Y;                       //Convert 1D index to 2D indeces
	cout << i << " " << j  << " " << k << " " << endl;
    if (i>0 && !visited[q=p-X]) {         //Visit -X neighbor
      visited[q]=1;
      if (VALID(i-1,j,k)) {dist[q]=dist[p]+1; queue.append(q);}
    }
    ...
    if (j>0 && !visited[q=p-Y]) {         //Visit -Y neighbor
      visited[q]=1;
      if (VALID(i,j-1,k)) {dist[q]=dist[p]+1; queue.append(q);}
    }
	...
	if (k>0 && !visited[q=p-Z]) {         //Visit -X neighbor
      visited[q]=1;
      if (VALID(i,j,k-1)) {dist[q]=dist[p]+1; queue.append(q);}
    ...
    if (k<xsize-1 && !visited[q=p+Z]) {   //Visit +X neighbor
      visited[q]=1;
      if (VALID(i,j,k+1)) {dist[q]=dist[p]+1; queue.append(q);}
